...
<body id="page-top">
            ...
            <div class="collapse navbar-collapse" id="navbarResponsive">
                <ul class="navbar-nav ms-auto">
                    ...
                    <?php if (!isset($_SESSION['id'])): ?>
                        <!-- Misafir kullanıcı için Giriş Yap/Kayıt Ol -->
                        <a class="nav-link" href="girisVekayit.php">Kayıt ol/Giriş Yap</a>
                    <?php else: ?>
                        <div style="display: flex; align-items: center;">
                  <span class="nav-link disabled" style="color: white; margin-right: 10px;">Hoş geldin <?php echo htmlspecialchars($_SESSION['userName']); ?></span>
                  <a class="nav-link" href="logout.php" style="color: white;">Çıkış Yap</a>
                       </div>
                    <?php endif; ?>
                    ...
                    <li class="nav-item">
                      <?php if (!isset($_SESSION['id'])): ?>
                            <!-- Misafir kullanıcı için pasif bağlantı -->
                            <a class="nav-link disabled" href="#" style="cursor: not-allowed; color: gray;" title="Randevu almak için giriş yapın.">Randevu Al</a>
                        <?php else: ?>
                            <!-- Giriş yapmış kullanıcı için aktif bağlantı -->
    ...
    <header class="masthead text-center text-white">
        <div class="masthead-content">
            <div class="container px-5">
                <h1 class="masthead-heading mb-0">Gelişmiş tedavi yöntemlerimizle</h1>
                <h2 class="masthead-subheading mb-5">Birlikte Sağlığa</h2>
            </div>
        </div>
    </header>
...
$sql = "SELECT * FROM doktorlar";
 ...
 if ($result->num_rows > 0) {
     ...
     while ($row = $result->fetch_assoc()) {
         ...
         echo '<a href="d_aciklama.php?id=' . $row["ID"] . '" class="btn btn-primary">Detayları Gör</a>'; 
         echo '</div>';
         echo '</div>';
         echo '</div>';
         $first = false;
...
if ($sorgu->num_rows > 0) {
    ...
    while ($row = $sorgu->fetch_assoc()) {
        ...
        echo '<a href="h_aciklama.php?id=' . $row['ID'] . '">';
